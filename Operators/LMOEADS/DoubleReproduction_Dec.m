function OffDec = DoubleReproduction_Dec(Problem,Pop,GuidingSolution,RefV)
...
% E-mail: shufen.qin@stu.tyust.edu.cn
    ...
    %% The First Phase
    OffPopX = GAonce(Problem,Pop.decs,GuidingSolution.decs); %遗传算法，将当前种群与引导解进行交叉
    OffPopX = unique(OffPopX,'rows'); %对交叉结果进行去重
    %OffPop  = Problem.Evaluation(OffPopX); %评估新生成的子代 OffPop
    % Pop  = Problem.Evaluation(OffPopX); %评估新生成的子代 OffPop
    OffDec = OffPopX;   % ❌ 不评估
    
    % % Environmental Selection
    % PopCom = [Pop,GuidingSolution,OffPop]; %种群合并：当前种群 Pop、引导解 GuidingSolution、新生成的子代 OffPop
    % [associate,Cosinemax] = Assign(PopCom,RefV); %将解分配到参考向量，计算余弦相似度
    % Ns = length(unique(associate)');
    % if Ns < (2/3) * Problem.N %如果覆盖的参考向量数量少于 2/3，说明解分布不均匀
    %     Pop = DominationSelection(Problem,PopCom); %分布不均匀 → 使用支配选择
    % else
    %     Pop = DecompositionSelection(Problem,PopCom,associate,Cosinemax); %分布均匀 → 使用分解选择
    % end
    % 
    % %% The Second Phase
    % %OffPopX = GA(Pop.decs);
    % OffPopX = GATwice(Pop.decs,Problem); %第二阶段的遗传算法，仅使用当前种群
